...
while True:
    ...
    if len(faces) > 0:
        ...
        if confirmation_done and similarity < threshold and len(faces) > 0:
            ...
            # Extract features of captured face images and save to list
            captured_faces_features.append(webcam_features_vector)

            # Stop capturing after 50 images
            if capture_count >= 50:
                print("Stopped capturing after 50 images.")
                break
...
# Convert list of face features to numpy array
captured_faces_features = np.array(captured_faces_features)

# Prompt user to enter name using GUI input dialog
if confirmation_done and name_entered:
    # Save face features to numpy file
